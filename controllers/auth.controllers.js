...
exports.signup = async (req, res) => {
  ...
  if (password.length < 6) {
    return res
      .status(400)
      .json({ result: false, message: "Le mot de passe doit contenir au moins 6 caractères." });
  }
...
exports.signin = async (req, res) => {
  ...
  if (!username || !password) {
    return res
      .status(400)
      .json({ result: false, message: "Veuillez saisir le nom d'utilisateur et le mot de passe." });
  }
  ...
  try {
    ...
    if (!user) {
      return res
        .status(401)
        .json({ result: false, message: "Identifiant ou mot de passe incorrect." });
    }
...
exports.addProfilePicture = async (req, res) => {
  ...
  try {
    ...
    const cloudinaryResult = await new Promise((resolve, reject) => {
      //Retire l'extension du fichier de son nom
    ...
    if (cloudinaryResult.secure_url) {
      const updateUserProfilePic = await User.findByIdAndUpdate(userId, {
    ...
    res.status(200).json({
      result: true,
      message: "L'image de profil à bien été modifié.",
    });
  } catch (error) {
    ...
    for (const post of userPosts) {
      if (post.hashtags.length > 0) {
        const isUpdated = await Promise.all(
          post.hashtags.map((tag) => Hashtag.updateOne({ name: tag }, { $inc: { count: -1 } })),
        );
      }
    }
