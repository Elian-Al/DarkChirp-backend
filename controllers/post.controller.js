    ...
    try {
        ...
        if (hashtagsArray.length > 0) {
            ...
            for (const tag of hashtagsArray) {
                await Hashtag.updateOne({ name: tag.toLowerCase() }, { $inc: { count: 1 } }, { upsert: true, new: true });
            }
        }
...
exports.getAllPosts = async (req, res) => {
    try {
        const allPosts = await Post.find().populate("user", "username firstname lastname profilePicture").sort({ createdAt: -1 });
...
exports.deletePost = async (req, res) => {
    ...
    try {
        ...
        await User.updateMany({ $or: [{ likedPosts: postId }, { savedPosts: postId }] }, { $pull: { likedPosts: postId, savedPosts: postId } });
