...
exports.createPost = async (req, res) => {
  ...
  if (!content || content.trim().length === 0) {
    return res
      .status(400)
      .json({ result: false, message: "Le contenu du post ne peut pas être vide." });
  }
...
exports.getAllPosts = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ result: false, message: "Erreur serveur lors de l'affichage du fil d'actualité." });
  }
};
  ...
  try {
    ...
    if (post.user.toString() !== userId) {
      return res.status(403).json({
        result: false,
        message: "Accès non autorisé. Vous ne pouvez supprimer que vos propres Posts.",
      });
    }
...
exports.savePost = async (req, res) => {
  ...
  try {
    ...
    if (!post || !user) {
      return res
        .status(404)
        .json({ result: false, message: "Ressource (Post ou Utilisateur) non trouvée." });
    }
    ...
    res
      .status(500)
      .json({ result: false, message: "Erreur serveur lors de l'opération de sauvegarde." });
  }
};
...
exports.getTrendingHashtag = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ result: false, message: "Erreur serveur lors de la récurpération des données." });
  }
};
...
exports.getPostsByHashtag = async (req, res) => {
  ...
  try {
    ...
    if (posts.length === 0) {
      return res
        .status(200)
        .json({ result: true, message: `Aucun Post trouvé pour #${hashtagName}.`, data: [] });
    }
...
exports.getAllUserPosts = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      result: false,
      message: "Erreur serveur lors de la récupération des Posts de l'utilisateur.",
    });
  }
};
...
exports.getLikedPosts = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ result: false, message: "Erreur serveur lors de la récupération des Posts." });
  }
};
